...
internal class KFieldBuilder(element: KSClassDeclaration, bindingManager: KBindingManager) :
    ...
    fun addProxyFields(classBuilder: TypeSpec.Builder) {
        ...
        classBuilder
            .addProperty(PropertySpec.builder("_binderID", Int::class, KModifier.PRIVATE)
                .initializer("__remoter_getStubID()").mutable().build())
            .addProperty(PropertySpec.builder("_stubProcess", Int::class, KModifier.PRIVATE)
                .initializer("__remoter_getStubProcessID()").mutable().build())
            .addProperty(PropertySpec.builder("proxyListener", ClassName.bestGuess("DeathRecipient").copy(true))
                .mutable().addModifiers(KModifier.PRIVATE).initializer("null").build())

        if (bindingManager.hasRemoterBuilder()) {
            classBuilder.addProperty(
                PropertySpec.builder("_remoterServiceConnector", ClassName("remoter.builder", "IServiceConnector").copy(true), KModifier.PRIVATE)
        ...
        classBuilder.addProperty(
            PropertySpec.builder("stubMap", ClassName("kotlin.collections", "MutableMap")
                .parameterizedBy(Any::class.asTypeName(), ClassName("android.os", "IBinder").copy(true)),
                KModifier.PRIVATE)
                .initializer("%T()", WeakHashMap::class).build()
        )
        classBuilder.addProperty(
            PropertySpec.builder("__global_properties", ClassName("kotlin.collections", "MutableMap")
    ...
    fun addStubFields(classBuilder: TypeSpec.Builder) {
        ...
        classBuilder.addProperty(
            PropertySpec.builder("binderWrapper", ClassName.bestGuess("BinderWrapper"))
                .initializer("BinderWrapper(this)").addModifiers(KModifier.PRIVATE).build()
        )
        classBuilder.addProperty(
            PropertySpec.builder("__processLastMethodMap", ClassName("kotlin.collections", "MutableMap")
                .parameterizedBy(Int::class.asTypeName(), Int::class.asTypeName()))
                .addModifiers(KModifier.PRIVATE).initializer("mutableMapOf()").mutable().build()
        )
        ...
        companion.addProperty(PropertySpec.builder("checkStubProxyMatch", Boolean::class)
            ...
            .mutable().initializer("true").build())
    ...
    private fun addCommonFields(classBuilder: TypeSpec.Builder) {
        classBuilder.addProperty(PropertySpec.builder("DESCRIPTOR", String::class)
            .addModifiers(KModifier.PRIVATE)
            .initializer("\"$remoterInterfacePackageName.$remoterInterfaceClassName\"").build())
        classBuilder.addProperty(PropertySpec.builder("REMOTER_EXCEPTION_CODE", Int::class)
            .addModifiers(KModifier.PRIVATE).initializer("-99999").build())
    }

    private fun addCommonFields(classBuilder: TypeSpec.Builder, member: KSFunctionDeclaration, methodIndex: Int) {
        val methodName = member.simpleName.asString()
        classBuilder.addProperty(
            PropertySpec.builder("TRANSACTION_${methodName}_$methodIndex", Int::class)
